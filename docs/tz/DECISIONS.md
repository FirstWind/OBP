...
## Appendix12 (шкалы баллов)
- В приложении №12 нет возрастной дифференциации шкал.
- Шкалы применяются ко всем возрастным группам соответствующего пола.
- Измерения шкал: sex × exercise × variant.

## Table 3 (Appendix12) варианты
- `exercise_id=34`: варианты `distance_1100` и `distance_3100` (дистанция полосы).
- `exercise_id=51` и `exercise_id=52`: варианты `unit_le_10` и `unit_gt_10` (численность подразделения).

## Appendix10 (перечень назначаемых упражнений)
- Упражнения №5–9 и №45 отсутствуют в приложении №10.
- Не включаем их в назначение по умолчанию в v1; допускаются только по отдельному нормативному указанию.
